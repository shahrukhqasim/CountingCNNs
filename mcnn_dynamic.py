...
class McnnNetworkDynamic:
    def __init__(self):
        ...
        self.image_width = int(config['mcnn_dynamic']['image_width'])
        self.image_height = int(config['mcnn_dynamic']['image_height'])
        self.learning_rate = float(config['mcnn_dynamic']['learning_rate'])
        self.from_scratch = int(config['mcnn_dynamic']['from_scratch']) == 1
        self.EPOCHS = int(config['mcnn_dynamic']['epochs'])
        self.PRETRAIN_EACH_EPOCHS = int(config['mcnn_dynamic']['pre_train_each_epochs'])
        self.full_model_path = str(config['mcnn_dynamic']['model_path'])
        self.data_path = str(config['mcnn_dynamic']['train_data_path'])
        self.test_data_path = str(config['mcnn_dynamic']['test_data_path'])
    ...
    def construct_graphs(self):
        regressor_input = self.regressor_input = tf.placeholder("float32", shape=[1, None, None, 3])
