...
class TrainDataLoader(implements(LoadInterface)):
    ...
    def load_datum(self, full_path):
        ...
        density_map = self.load_map(os.path.join(full_path, 'density.dat'), loaded_image_height, loaded_image_width)
        flow_magnitude = self.load_map(os.path.join(full_path, 'flow_magnitude.dat'), loaded_image_height, loaded_image_width)
        flow_angle     = self.load_map(os.path.join(full_path, 'flow_angle.dat'),     loaded_image_height, loaded_image_width)
        image = np.concatenate((image, np.expand_dims(flow_magnitude,axis=2)), axis=2)
        image = np.concatenate((image, np.expand_dims(flow_angle    ,axis=2)), axis=2)

        sum = np.sum(density_map)

        # image = cv2.resize(image, (self.image_height * 3, self.image_width * 3))
        density_map = cv2.resize(density_map, (int(loaded_image_width / 4), int(loaded_image_height / 4)))
        sum_2 = np.sum(density_map)
        density_map = (sum / sum_2) * density_map

        h = int(loaded_image_height / 3)
        w = int(loaded_image_width / 3)
...
class McnnNetworkDynamic5Channel:
    def __init__(self):

        config = cp.ConfigParser()
        config.read('settings.ini')

        self.image_width = int(config['mcnn_dynamic_5_channel']['image_width'])
        self.image_height = int(config['mcnn_dynamic_5_channel']['image_height'])
        self.learning_rate = float(config['mcnn_dynamic_5_channel']['learning_rate'])
        self.from_scratch = int(config['mcnn_dynamic_5_channel']['from_scratch']) == 1
        self.EPOCHS = int(config['mcnn_dynamic_5_channel']['epochs'])
        self.PRETRAIN_EACH_EPOCHS = int(config['mcnn_dynamic_5_channel']['pre_train_each_epochs'])
        self.full_model_path = str(config['mcnn_dynamic_5_channel']['model_path'])
        self.data_path = str(config['mcnn_dynamic_5_channel']['train_data_path'])
        self.test_data_path = str(config['mcnn_dynamic_5_channel']['test_data_path'])
    ...
    def construct_graphs(self):
        regressor_input = self.regressor_input = tf.placeholder("float32", shape=[1, None, None, 5])
